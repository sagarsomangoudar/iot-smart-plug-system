...
function Header(props) {
	var ths = props.groupNames.map(groupName => {
		var onClick = () => props.onAddAllMembers(groupName);
		return <th key={groupName} className="text-center">
	...
	return (
		<thead>
			<tr>
				...
				<th colSpan={props.groupNames.length} className="text-center">Groups</th>
...
function Body(props) {
	var rows = props.members.get_member_names().map(memberName =>
		<Row key={memberName} memberName={memberName} members={props.members}
			...
			onDeleteMemberFromAllGroups={props.onDeleteMemberFromAllGroups}/>);
	...
	return (
		<table className="table table-striped table-bordered">
			<Header groupNames={props.members.get_group_names()}
					onAddAllMembers={props.onAddAllMembers}/>   
